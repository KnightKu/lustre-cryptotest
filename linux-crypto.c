				 ...
				 unsigned int key_len)
{
	...
	*type = cfs_crypto_hash_type(hash_alg);
	...
	if (*type == NULL) {
		printk("Unsupported hash algorithm id = %d, max id is %d\n",
	...
	if (IS_ERR(hdesc->tfm)) {
		printk("Failed to alloc crypto hash %s\n",
		       (*type)->cht_name);
	...
	/*
	printk("Using crypto hash: %s (%s) speed %d MB/s\n",
...
out_err:
	if (err != 0) {
		...
		printk("Crypto hash algorithm %s test error: rc = %d\n",
		...
		printk("Crypto hash algorithm %-9s speed = %d MB/s\n",
...
static int cfs_crypto_test_hashes(void)
{
	...
	for (hash_alg = CFS_HASH_ALG_ADLER32; hash_alg < CFS_HASH_ALG_MAX; hash_alg++)
